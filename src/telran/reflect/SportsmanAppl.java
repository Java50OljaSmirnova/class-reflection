...
public class SportsmanAppl {
	...
	public static void main(String[] args) throws Exception {
		if(args.length == 0) {
			System.out.println("Usage <name of class as an arguments>");
		} else {
			Class<Sportsman> clazz = (Class<Sportsman>) Class.forName(BASE_PACKAGE + args[0]);
			Constructor<Sportsman> constructor = args.length > 1 ? clazz.getConstructor(String.class) : clazz.getConstructor();
